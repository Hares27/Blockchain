...
Blockchain.prototype.createNewBlock = function (Block) {
  const newBlock = {
    ...
    currentBlockHash: Block.currentHash,
    nonce: Block.nonce,
  };
...
Blockchain.prototype.proofOfwork = function (previousBlockHash) {
  ...
  let Hash = this.hashBlock({
    ...
    blocktransactions: this.pendingtransactions,
    nonce: nonce,
  }).toString();
...
Blockchain.prototype.hashBlock = function (block) {
  const blockdataAsString =
    block.previousBlockHash +
    JSON.stringify(block.blocktransactions) +
